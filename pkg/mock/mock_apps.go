...
func addMotorApp(model *kube.InMemoryModel) {
	addAppWithSpecs(model, appSpec{
		...
		envVars: []corev1.EnvVar{
			...
			{Name: "LDAP_SECRET", ValueFrom: &corev1.EnvVarSource{SecretKeyRef: &corev1.SecretKeySelector{LocalObjectReference: corev1.LocalObjectReference{Name: "petshop-tennant-secrets"}, Key: "PSB-TENNANT-PASSWORD"}}},
			{Name: "LDAP_USER", Value: "petshop-tennant-sa"},
			{Name: "LOGGING_LEVEL", Value: "INFO"},
			{Name: "NEO4J_PASSWORD", ValueFrom: &corev1.EnvVarSource{SecretKeyRef: &corev1.SecretKeySelector{LocalObjectReference: corev1.LocalObjectReference{Name: "petshop-tennant-secrets"}, Key: "PSB-DATABASE-PASSWORD"}}},
...
func addWebApp(model *kube.InMemoryModel) {
	addAppWithSpecs(model, appSpec{
		...
		envVars: []corev1.EnvVar{
			...
			{Name: "NEO4J_PASSWORD", ValueFrom: &corev1.EnvVarSource{SecretKeyRef: &corev1.SecretKeySelector{LocalObjectReference: corev1.LocalObjectReference{Name: "petshop-frontend-girls-secrets"}, Key: "PSB-DATABASE-PASSWORD"}}},
			{Name: "NEO4J_URI", Value: "bolt://petshop-db-service:7687"},
			{Name: "NEO4J_USERNAME", Value: "neo4j"},
			{Name: "OIDC_CLIENT_ID", Value: "petshop-frontend-girls"},
			{Name: "OIDC_CLIENT_SECRET", ValueFrom: &corev1.EnvVarSource{SecretKeyRef: &corev1.SecretKeySelector{LocalObjectReference: corev1.LocalObjectReference{Name: "petshop-frontend-girls-secrets"}, Key: "PSB-OIDC-SECRET"}}},
