...
```bash
# JSON graph
curl "http://localhost:8080/api/graph?context=in-cluster&selector=deployment/myapp/frontend&namespace=default"

# JSON tree
curl -H "Accept: application/json" "http://localhost:8080/api/tree?context=in-cluster&namespace=production&selector=pod/production/myapp"
